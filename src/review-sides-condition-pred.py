...
results_dir = os.path.join(current_dir, 'results', 'review-sides-condition-pred')
...
def plot_predictions_vs_actual(y_test, y_pred, target_name, model_name):
    ...
    plt.title(f'Model: {model_name} | Target: {target_name}\nPredicted vs. Actual Values (Reviews + Sides + Condition)', fontsize=12)
    plt.grid(True)
...
def plot_residuals(residuals, y_pred, target_name, model_name):
    ...
    plt.title(f'Model: {model_name} | Target: {target_name}\nResiduals vs. Predicted Values (Reviews + Sides + Condition)', fontsize=12)
    plt.grid(True)
    ...
    plt.title(f'Model: {model_name} | Target: {target_name}\nDistribution of Residuals (Reviews + Sides + Condition)', fontsize=12)
    plt.grid(True)
    ...
    for model_name, model_results in results.items():
        for target, result in model_results.items():
            metrics_data.append({
                ...
                'PredictTime': result['metrics']['PredictTime']
            })
...
def main():
    ...
    # Handle missing values in all text fields
    ...
    print("Creating TF-IDF features for Reviews, Side Effects, and Condition...")
    tfidf_reviews = TfidfVectorizer(max_features=4000)
    tfidf_sides = TfidfVectorizer(max_features=1000)  # Fewer features for side effects
    tfidf_condition = TfidfVectorizer(max_features=500)  # Even fewer features for condition as it's typically very short
    
    # Transform all text fields
    ...
    X_condition_tfidf = tfidf_condition.fit_transform(df['processed_condition'])
    
    # Combine features by horizontally stacking all three matrices
    X_combined = hstack([X_reviews_tfidf, X_sides_tfidf, X_condition_tfidf])
    ...
    for model_name, model in models.items():
        ...
        for target in targets:
            print(f"Training {model_name} for {target} using Reviews + Side Effects + Condition...")
            # Split data
