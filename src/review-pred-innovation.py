...
def main():
    ...
    embed_file_path = os.path.join(results_dir, 'sentence_embeddings.pt')
    ...
    for model_name, model in models.items():
        ...
        print(f"Training {model_name} for all targets...")
        for target in targets:
            # Split data
            print(f"Training {model_name} for {target}...")
            X_train, X_test, y_train, y_test = train_test_split(
                X_embed, df[target], test_size=0.2, random_state=42
            )
            ...
            print(f"Training {model_name} for {target} completed.")
        print(f"Training {model_name} for all targets completed.")
